...
our $targetvrt2         = $dir . $targetVrtFile2. ".vrt";
    ...
    # removeIconsAndTextboxesInMaskedAreas( "Obstacle TextBox",        \%latitudeTextBoxes );
  ...
  our %gcps = ();
    ...
    # #Find all obstacles within our defined distance from the airport that have a height in the list of potential latitudeTextBoxes and are unique
    # our %unique_obstacles_from_db = ();
    # our $unique_obstacles_from_dbCount;
    # findObstaclesNearAirport( \%unique_obstacles_from_db );

    # #Try to find closest obstacleTextBox center to each obstacleIcon center and then do the reverse
    # findClosestBToA( \%obstacleIcons,     \%latitudeTextBoxes );
    # findClosestBToA( \%latitudeTextBoxes, \%obstacleIcons, );

    # #Make sure there is a bi-directional match between icon and textbox
    # #Returns a reference to a hash which combines info from icon, textbox and database
    # my $matchedObstacleIconsToTextBoxes =
    # joinIconTextboxAndDatabaseHashes( \%obstacleIcons, \%latitudeTextBoxes,
    # \%unique_obstacles_from_db );
...
# ^Q$/m;
    ...
    # if ( $tempObstacles_length >= $dataPointsPerObstacleIcon ) {
        ...
        # {
            ...
            # $main::obstacleIcons{ $i . $rand }{"latitudeTextBoxesThatPointToMe"}
              # = 0;
...
# #print $latitudeTextBoxes{$key2}{"Text"} . "$key\n";
...
# if (
# exists
# $main::unique_obstacles_from_db{ $main::latitudeTextBoxes{$key}
# {"Text"} } )
# {
